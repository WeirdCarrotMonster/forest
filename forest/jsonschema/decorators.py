...
def schema(argument=None):
    ...
    Файлы с описанием декораторов ищутся в поддиректории schema, по имени без расширения.
    Если имя отсутствует, декоратор подставляет пустую схему, валидация которой проходит
    всегда, если во входных данных словарь. Такая схема автоматически преобразует его в
    kwargs запроса.
